document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/health-data')
        ...
        .then(data => {
            ...
            const filteredData = data.filter(item => new Date(item.DateTime) >= oneHourAgo);
            const labels = filteredData.map(item => new Date(item.DateTime).toISOString());
            const heartRates = filteredData.map(item => item.HealthData.HeartRate);
            const oxygenLevels = filteredData.map(item => item.HealthData.OxygenLevel);
            ...
            socket.on('healthDataUpdate', function(updatedData) {
                ...
                if (updatedData.HealthData) {
                    ...
                    if (updatedTime >= oneHourAgo) {
                        ...
                        // Update chart data
                        healthChart.data.labels.push(updatedTimeISO);
                        healthChart.data.datasets[0].data.push({ x: updatedTimeISO, y: updatedData.HealthData.HeartRate });
                        healthChart.data.datasets[1].data.push({ x: updatedTimeISO, y: updatedData.HealthData.OxygenLevel });
                        healthChart.update();

                        // Update table data
                        const newRow = document.createElement('tr');
                        newRow.innerHTML = `
                            <td>${updatedTimeISO}</td>
            ...
            $('#daterange').daterangepicker({
                ...
                filteredLabels.forEach((label, index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${filteredData[index].HealthData.AccelerometerData.Ax}</td>
                        <td>${filteredData[index].HealthData.AccelerometerData.Ay}</td>
                        <td>${filteredData[index].HealthData.AccelerometerData.Az}</td>
                    `;
                    tableBody.appendChild(row);
                });
            });
